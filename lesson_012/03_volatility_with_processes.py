...
class FilesOpenForCalculatingVolatility(multiprocessing.Process):

    def __init__(self, path_name, collector, *args, **kwargs):
    ...
    def run(self):
        try:
            ...
            self.collector.put((self.list_volatility, self.list_zero_volatility))
...
def main():
    ...
    calculating_files = FilesOpenForCalculatingVolatility(path_name="trades", collector=collector)
    calculating_files.start()
    while True:
        try:
            list_volatility, list_zero_volatility = collector.get(timeout=1)
        except queue.Empty:
            if not calculating_files.is_alive():
                break
    calculating_files.join()

    print('Максимальная волатильность:');
    for max_v in list_volatility[:-4:-1]:
        print(max_v)
    print('Минимальная волатильность:');
    for min_v in list_volatility[2::-1]:
        print(min_v)
    print('Нулевая волатильность:')

    result = [x for x in list_zero_volatility]
    print(result)
